...
import adept.repository.models.{Commit, RankId, Ranking, VariantHash}
import adept.resolution.models.Id

//Evaluate whether is safe enough to use iterators as a perf improvement
case class RankingMetadata(variants: Seq[VariantHash]) {
  ...
  def write(id: Id, rankId: RankId, repository: Repository): File = {
    ...
    if (variants.distinct != variants)
      throw new Exception("Cannot write ranking file because there several similar rankids: " + id +
        " rank id: " + rankId + " in " + repository.dir.getAbsolutePath)
...
object RankingMetadata {
    ...
    repository.usingFileInputStream(file) {
      ...
      case Left(error) =>
        throw new Exception("Could not read file: " + file.getAbsolutePath + " for id: " + id +
          " rank id: " + rankId + ". Got error: " + error)
    }
  }
  ...
  def read(id: Id, rankId: RankId, repository: GitRepository, commit: Commit): Option[RankingMetadata] = {
    repository.usingRankingInputStream(id, rankId, commit) {
      case Right(Some(is)) =>
        Some(RankingMetadata(scala.io.Source.fromInputStream(is).getLines().map { line =>
          VariantHash(line.trim())
        }.toSeq))
      case Right(None) => None
      case Left(error) =>
        throw new Exception("Could not read ranking file for: " + id + ", " + rankId + " for commit: " +
          commit + " in dir:  " + repository.dir + ". Got error: " + error)
  ...
  def listRankIds(id: Id, repository: GitRepository, commit: Commit): Set[RankId] = {
    ...
    val rankIds = repository.usePath[RankId](Some(rankPath), commit) {
  ...
  def listRankIds(id: Id, repository: Repository): Set[RankId] = {
    //do not match on / because commit version does not do that either
    val RankIdExtractionRegEx = s"""([^/]*?).$RankingFileEnding$$""".r
