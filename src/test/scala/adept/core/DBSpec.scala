...
class DBSpec extends FreshDBEachRun with ShouldMatchers {
  ...
  def expectLeft[A, B](res:Either[A, B], reason: String) = {
    assert(res.isLeft, "Expected Right, but got: "+res + ". Expected a Left because: "+reason)
  }
  ...
  describe("Adding modules") {
    it("should be idempotent") {
      ...
      expectLeft(Adept.commit(repoName), "having nothing to commit because the one above failed")
    }
  }
  describe("Commiting modules") {
    it("should only be possible if dependencies are already there") {
      ...
      expectLeft(Adept.commit(repoName), "there are missing deps in the line above")
  ...
  describe("Updating modules") {
    it("should work as expected") {
      ...
      expectRight(Adept.update(repoName, parent.hash, parent.copy(deps = modules(parent).map(_.hash))), "")
      val commitedRes = Adept.commit(repoName)
