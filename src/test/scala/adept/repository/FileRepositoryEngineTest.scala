...
class RepositoryEngineTest extends FunSuite with MustMatchers {
  ...
  test("basic variant read/write file repo engine test") {
    import EitherValues._
    usingTempDir { tmpDir =>
      val repo = new FileRepository(tmpDir, "test-repo")
      ...
      val manager = new FileRepositoryEngine(tmpDir, Set(repo))
  ...
  test("basic artifact read/write file repo engine test") {
    import EitherValues._
    usingTempDir { tmpDir =>
      val repo = new FileRepository(tmpDir, "test-repo")
  ...
  //TODO: move benchmarks to somewhere else so that tests doesn't take too long
  test("basic file bench") {
    import EitherValues._
    usingTempDir { tmpDir =>
      val repo = new FileRepository(tmpDir, "test-repo")
      
      println(timer(for (i <- 1 to 1000) {
