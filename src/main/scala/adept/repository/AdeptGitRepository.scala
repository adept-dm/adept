...
//  val ChangesFileName = "changes"
//  val ReposDirName = "repos"
//  val RepositoryMetdataDirName = "repos"
//
...
//  def getChangesFile(baseDir: File, name: String) = {
//    val rewritesDir = new File(getRepoDir(baseDir, name), RewritesDirName)
//    new File(rewritesDir, ChangesFileName)
//  }
//
//}
//
...
//  val GitRemoteRepositoryName = "origin"
//
...
//    else {
//      val remoteCommitString = "remotes/" + GitRemoteRepositoryName + "/" + commitString
...
//    var variantMetadata = Set.empty[VariantMetadata]
...
//            try {
//              if (currentPath.startsWith(VariantsDirName)) variantMetadata += VariantMetadata.fromJson(reader)
...
//    }
//    MetadataContent(variantMetadata, repositoryMetadata)
//  }
//
...
//  }
//
//  private def updateBranch(removals: MetadataContent => Seq[File], additions: MetadataContent => Seq[File], commitMsg: String, branch: String = workingBranchName, commit: Commit = Commit(Head)): Commit = {
//    {
...
//        new AdeptCommit(this, updateBranch(removals, additions, commitMsg, branch, mostRecentCommit.commit))
//      } else  {
//        git.checkout.setName(branch).call()
//        val metadataBranchRevCommit = usingRevWalk { (gitRepo, revWalk) =>
//          lookup(gitRepo, revWalk, commit.value)
//        }
//        println(metadataBranchRevCommit + " for " + commit.value)
//        val cherryPickCommits = usingRevWalk { (gitRepo, revWalk) => //commits that will be cherry picked later
//          revWalk.markStart(revWalk.lookupCommit(gitRepo.resolve(Constants.HEAD)))
//          val it = revWalk.iterator()
//          var commits: Seq[RevCommit] = Seq.empty
//          var finished = false
//          
//          while (it.hasNext && !finished) {
//            val revCommit = it.next()
//            if (revCommit.name == metadataBranchRevCommit.name) {
//              finished = true
//            } else {
//              commits :+= revCommit
//            }
...
//          commits.reverse
//        }
//        val tmpBranchName = "tmp-branches/"+commit.value
//        val parkedBranchName = "parked-branches/"+commit.value
//        git.branchCreate().setStartPoint(metadataBranchRevCommit.name).setName(tmpBranchName).call()
//        
//        updateBranch(removals, additions, commitMsg, tmpBranchName, commit)
//        
////        val newBranchFileGitPath : String = ??? //points to parked branch branch
////        git.add().addFilepattern(newBranchFileGitPath).call()
//        
//        val commitMap = cherryPickCommits.map { commit =>
//          commit -> git.cherryPick().include(commit).call().getNewHead()
//        }.toMap
//        
//        git.checkout().setName(branch).call()
////        val oldBranchFileGitPath : String = ??? 
////        git.add().addFilepattern(oldBranchFileGitPath).call()
////        val parkedCommit = git.commit().setMessage("Parked branch").call()
//        
//        git.branchRename().setNewName(parkedBranchName).setOldName(branch).call() //TODO: is this too risky (if it fails I mean)?
//        val ref = git.branchRename().setNewName(branch).setOldName(tmpBranchName).call()
//        AdeptCommit(this, Commit(ref.getName()))
//      }
//    } finally {
//      git.checkout().setName(prevBranch).call()
//    }
//  }
//
//  /**
