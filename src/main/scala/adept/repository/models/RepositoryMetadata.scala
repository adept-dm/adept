...
object RepositoryMetadata {
  ...
  def file(repository: AdeptGitRepository, id: Id, variant: Hash): File = {
    repository.getRepositoryMetadataFile(id, variant)
  }

}


case class RepositoryInfo(name: String, commit: Commit)
...
case class RepositoryMetadata(id: Id, variant: Hash, configurations: Seq[RepositoryConfiguration]) {
  def load(baseDir: File, id: Id, configuration: ConfigurationId): Set[AdeptCommit] = { //TODO: this will be replaced with something smarter that actually loads repositories. it should probably not be in this file either btw
    configurations.filter(_.id == configuration).flatMap{ repoConf =>
      repoConf.info.map{ repoInfo =>
        AdeptCommit(new AdeptGitRepository(baseDir, repoInfo.name), repoInfo.commit)
      }
    }.toSet
  }
  ...
  def write(repository: AdeptGitRepository): File = {
    MetadataContent.usingFileWriter(RepositoryMetadata.file(repository, id, variant))(toJson)
  }
}